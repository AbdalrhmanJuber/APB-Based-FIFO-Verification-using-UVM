...
module apb_sync_fifo #(
  ...
  // ---- registers ----
  // en for turning the fifo on or of
  // clr for resets fifo contents and pointers
  // drop_empty_th defines what happens if CPU pushes when fifo is full
  ...
  always @(negedge PRESETn or posedge PCLK) begin
    if (!PRESETn) begin
      ...
      if (PSEL && PENABLE && !PWRITE) begin // bug any read clears erro flags!!
        overflow <= 1'b0;
        underflow <= 1'b0;
      end
    end
  ...
  always_comb begin
    PRDATA = 32'd0;
    unique case (PADDR)
      STATUS_O: PRDATA = {18'd0, count[7:0], underflow,
                    overflow, almost_empty, almost_full, full, empty};
  ...
  // Count/flags
  always @(posedge PCLK or posedge PRESETn) begin //bug should be negedge reset
    if (PRESETn) begin // should be !PRESETn
      count <= '0;
      wptr  <= '0;
      rptr  <= '0;
      empty <= 1'b1;
      ...
      if (clr) begin
        ...
        // Update empty/full
        empty <= (count == 0); // bug uses old count
        full  <= (count == DEPTH-1);// bug should be count == DEPTH
      end
    end
  end
